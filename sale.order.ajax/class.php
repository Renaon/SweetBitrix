<?php
require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/disnuts/index.php");
...
class SaleOrderAjax extends \CBitrixComponent
{
	...
	protected function obtainTotal()
	{
        ...
        $disnuts = new Disnuts($basket->getBasePrice());
        $this->bonucePay =  $disnuts->discActions();
...
//		); //значение float
        ...
        echo(Loc::getMessage("FINAL_SALE").$this->bonucePay);
        $arResult['DISCOUNT_PRICE'] = Sale\PriceMaths::roundPrecision(
            $this->order->getDiscountPrice() + ($arResult['PRICE_WITHOUT_DISCOUNT_VALUE']  - $arResult['ORDER_PRICE'])
        ); //значение float
